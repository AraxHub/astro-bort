-- Таблица payments для хранения платежей (финальная версия со всеми полями)
CREATE TABLE IF NOT EXISTS payments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES tg_users(id) ON DELETE CASCADE,
    bot_id VARCHAR(255) NOT NULL DEFAULT '',      -- ID бота для мультибота
    ...
    provider_id VARCHAR(255) NOT NULL DEFAULT '', -- ID в системе провайдера (invoice_id для Stars)
    status VARCHAR(20) NOT NULL DEFAULT 'pending', -- pending, succeeded, failed, refunded
    product_id VARCHAR(100) NOT NULL,            -- что куплено (например, "premium_access", "monthly_feed")
...
-- Индекс для поиска по payload в metadata (функциональный B-tree индекс)
