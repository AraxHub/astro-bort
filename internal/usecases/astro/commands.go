...
func (s *Service) HandleCommand(ctx context.Context, user *domain.User, command string, updateID int64) error {
	switch command {
	case "start":
		return s.HandleStart(ctx, user, updateID)
	case "help":
		return s.HandleHelp(ctx, user)
	case "my_info":
		return s.HandleMyInfo(ctx, user)
	case "reset_birth_data":
		return s.HandleResetBirthData(ctx, user)
	default:
		return s.sendMessage(ctx, user.TelegramChatID, fmt.Sprintf("❌ Неизвестная команда: /%s\n\nИспользуй /help для списка команд", command))
	}
}

func (s *Service) HandleStart(ctx context.Context, user *domain.User, updateID int64) error {
	...
	if user.BirthDateTime == nil {
		...
		return s.sendMessage(ctx, user.TelegramChatID, message)
	}
	...
	if len(user.NatalChart) == 0 {
		...
		if err := s.fetchAndSaveNatalChart(ctx, user); err != nil {
			...
			return s.sendMessage(ctx, user.TelegramChatID, "❌ Не удалось получить натальную карту. Попробуйте позже.")
		}
	}
...
func (s *Service) HandleHelp(ctx context.Context, user *domain.User) error {
...
func (s *Service) HandleMyInfo(ctx context.Context, user *domain.User) error {
	...
	return s.sendMessage(ctx, user.TelegramChatID, message.String())
}

// HandleResetBirthData обрабатывает команду /reset_birth_data
func (s *Service) HandleResetBirthData(ctx context.Context, user *domain.User) error {
